...
router.post('/signup',body("email"),body('password').isLength({min:8,max:50}).withMessage('type valid password'),body('fullname').isString().isLength({min:6,max:30}).withMessage('type valid fullname'),body('phoneNumber').isNumeric().isLength({min:10,max:10}).withMessage('type valid phone number'),authController.signUp)
...
router.post('/ban',verifyToken,verifyAdmin,authController.ban)
router.post('/disBan',verifyToken,verifyAdmin,authController.disBan)
router.put('/status',verifyToken,verifyAdmin,authController.changeUserStates)
